...
public class Solution4 {
    ...
    public static double findMedianSortedArrays2(int[] nums1, int[] nums2) {
        ...
        int middleLeft = (len1 + len2 + 1) / 2;
        double midVal = getKthMin(nums1, 0, len1 - 1, nums2, 0, len2 - 1, middleLeft);

        if ((len1 + len2) % 2 == 0) {
            int middleRight = (len1 + len2 + 2) / 2;
            double midVal2 = getKthMin(nums1, 0, len1 - 1, nums2, 0, len2 - 1, middleRight);
            return (midVal + midVal2) / 2D;
        } else {
            return midVal;
        }
    }

    private static double getKthMin(int[] nums1, int start1, int end1, int[] nums2, int start2, int end2, int k) {
        int len1 = end1 - start1 + 1;
        int len2 = end2 - start2 + 1;

        if (len2 < len1) return getKthMin(nums2, start2, end2, nums1, start1, end1, k);
        if (len1 == 0) return nums2[start2 + k - 1];
